...
class DHPCompatiblePendulumEnv(gym.Env):
    ...
    def desired_trajectory(self, traj="fixed"):
        ...
        if traj=="fixed" or self.steps == 0:  # Initial training phase
            ...
            self.theta_desired = np.pi + 0.5 * (np.sin(0.006 * np.pi * t) + np.cos(0.004 * np.pi * t))
            self.theta_desired = ((self.theta_desired + np.pi) % (2 * np.pi)) - np.pi
            self.velocity_desired = 0.5 * (0.006 * np.pi * np.cos(0.006 * np.pi * t) - 
                                               0.004 * np.pi * np.sin(0.004 * np.pi * t))
    ...
    def reset(self, seed=None, options=None):
        ...
        self.x1 = 0.0  # Start near upright position
