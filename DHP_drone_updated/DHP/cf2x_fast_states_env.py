...
class CF2X_FastStates_HoverAviary(HoverAviary):
    ...
    def _observationSpace(self):
        """
        Override observation space to return only fast states
        Based on CF2X physical limits and DSL PID operational ranges
        
        Returns:
            Box space for 8 fast states: [z, roll, pitch, yaw, vz, wx, wy, wz]
        """
        # Fast states bounds based on CF2X physical limits and DSL PID operation
    ...
    def _computeObs(self):
        """
        Override observation computation to return only fast states
        
        Returns:
            np.array: Fast states [z, roll, pitch, yaw, vz, wx, wy, wz]
        """
        # Get full drone state vector (20 elements)
        full_state = self._getDroneStateVector(0)
        
        # Extract fast states (indices based on _getDroneStateVector format)
        ...
        # Store current position for reference generation
        self.current_pos = full_state[0:3]
        # Store current velocity (vx, vy, vz) for PD control
        self.current_vel = full_state[10:13]  # velocity components
    ...
    def generate_reference(self, target_pos):
        ...
        # # Debug: Print reference info every 100 calls (reduce spam)
        # if not hasattr(self, 'ref_call_count'):
        #     self.ref_call_count = 0
        # self.ref_call_count += 1
        
        # if self.ref_call_count % 100 == 0:
        #     pos_error_norm = np.linalg.norm(pos_e)
        #     print(f"[DEBUG] Ref #{self.ref_call_count}: pos_err={pos_error_norm:.4f}, "
        #           f"target_thrust_norm={np.linalg.norm(target_thrust):.4f}, "
        #           f"target_euler=[{target_euler[0]:.3f}, {target_euler[1]:.3f}, {target_euler[2]:.3f}], "
        #           f"vz_ref={vz_ref:.3f}")
    ...
    def compute_dhp_cost(self, state, reference):
        ...
        # if self.cost_call_count % 100 == 0:
        #     error_norm = np.linalg.norm(error)
        #     gradient_norm = np.linalg.norm(cost_gradient)
            # print(f"[DEBUG] Cost #{self.cost_call_count}: error_norm={error_norm:.4f}, "
            #       f"cost={cost:.2f}, gradient_norm={gradient_norm:.4f}, "
            #       f"pos_error={error[0]:.4f}")
    ...
    def step(self, action):
        ...
        """
        # Update current time for finer trajectory resolution (60 Hz)
        # Note: This creates 2x finer trajectory sampling than control frequency
        self.current_time += 1.0 / 60.0  # dt = 1/60 seconds (2x finer than CTRL_FREQ)
        ...
        # Add DHP-specific information using current target
